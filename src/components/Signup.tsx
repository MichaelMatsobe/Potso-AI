...
import { Mail, Lock, User, Loader2, Eye, EyeOff, AlertCircle, ArrowRight } from 'lucide-react';
...
export const Signup = ({ onSwitchToLogin, onSignupSuccess }: SignupProps) => {
  ...
  const handleGoogleSignUp = async (response: any) => {
    try {
      ...
      if (response.credential) {
        ...
        showNotification('success', 'Account Created', 'Redirecting to chat...');
        setTimeout(() => {
          onSignupSuccess?.();
        }, 1000);
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate form
    const validation = validateSignupForm(email, password, displayName, confirmPassword);
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full bg-white/10 border rounded-lg pl-10 pr-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary transition-all ${
                    errors.displayName ? 'border-red-500/50 focus:border-red-500' : 'border-white/20 focus:border-primary'
                  }`}
                />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className={`w-full bg-white/10 border rounded-lg pl-10 pr-10 py-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary transition-all ${
                    errors.confirmPassword ? 'border-red-500/50 focus:border-red-500' : 'border-white/20 focus:border-primary'
                  }`}
                />
                <button
                  type="button"
